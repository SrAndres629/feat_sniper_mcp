...
# Pydantic y pyiceberg imprimen warnings durante el import.
# Esto corrompe el canal STDIO de MCP. Solución: Agujero negro temporal.
_original_stdout = sys.stdout
_original_stderr = sys.stderr
_devnull = io.StringIO() # Blackhole

sys.stdout = _devnull
sys.stderr = _devnull  # CRITICO: pyiceberg escribe a stderr, no stdout
# === FIN BLACKHOLE TEMPORAL ===

# 1. Desactivar OpenTelemetry (Causante del crash I/O closed)
os.environ["OTEL_TRACES_EXPORTER"] = "none"
os.environ["OTEL_METRICS_EXPORTER"] = "none"
os.environ["OTEL_LOGS_EXPORTER"] = "none"
os.environ["OTEL_CONSOLE_EXPORT"] = "0"

# 2. Silenciar Warnings de Python AGRESIVAMENTE (pyiceberg, pydantic, etc.)
import warnings
...
# 3. Configurar Logging Global para que sea SILENCIOSO en consola
import logging
try:
...
# 4. IMPORTAR TODAS LAS LIBRERIAS PELIGROSAS (pydantic, fastmcp, etc.)
import asyncio

# Parche de Windows para ZMQ
if sys.platform == 'win32':
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())

# Hijack dummy for compatibility (Does nothing dangerous now)
try:
    from app.core.logger import hijack_streams
    hijack_streams(mcp_mode=True)
except Exception:
    pass
...
# Restaurar flujos SOLO DESPUES de que todas las librerías estén importadas
sys.stdout = _original_stdout
sys.stderr = _original_stderr
# === SISTEMA LISTO PARA OPERAR EN MODO MCP ===
...
# Inicializar FastMCP (El logging se controla via PYTHONWARNINGS al inicio del script)
...
@mcp.tool()
@pulse_observer
async def get_fractal_physics(symbol: str, timeframe: str = "M5"):
    """
    Analiza la 'Física de Capas' del mercado:
    ...
    # Necesitamos suficientes velas para L4 (2048)
    df_raw = await market.get_candles(symbol, timeframe, n_candles=settings.LAYER_BIAS_PERIOD + 10)
    
    if df_raw is None or (isinstance(df_raw, pd.DataFrame) and df_raw.empty) or (isinstance(df_raw, list) and not df_raw):
    ...
    if physics_df.empty:
        return {"error": "Insufficient data for fractal calculation"}
        
    latest = physics_df.iloc[-1].to_dict()
    
    # Añadir interpretación básica
    interpretation = "NEUTRAL"
    if latest['L1_Width'] < physics_df['L1_Width'].rolling(100).mean().iloc[-1] * 0.5:
        interpretation = "HIGH_COMPRESSION_VOLATILITY_ALERT"
    
    return {
        "symbol": symbol,
        "timeframe": timeframe,
        "physics": {k: round(v, 6) for k, v in latest.items()},
...
if __name__ == "__main__":
    # Detectar si estamos en Docker/Linux via variable de entorno (ms confiable para MCP)
    is_docker = os.environ.get("DOCKER_MODE", "").lower() == "true"
    
    if is_docker:
        # En Docker usamos SSE por defecto
        mcp.run(transport="sse", host="0.0.0.0", port=8000)
    else:
        # En Windows/Local, VS Code espera STDIO (Tubería). 
        # IMPORTANTE: NO imprimir nada en stdout antes de mcp.run()
        mcp.run()
