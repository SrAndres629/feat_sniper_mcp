...
class FeatProcessor:
    ...
    def process_ticks_to_ohlcv(self, ticks_df: pd.DataFrame, timeframe: str = '15T') -> pd.DataFrame:
        """
        ...
        vol = ticks_df['last_size'].resample(timeframe).sum().rename('volume') # Real volume proxy
        tick_count = ticks_df['last_price'].resample(timeframe).count().rename('tick_volume')
    ...
    def apply_feat_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.join(accel_features[['disp_norm', 'vol_z', 'candle_momentum', 'rvol', 'cvd_proxy', 'accel_score', 'accel_flag']])
        ...
        df['atr14'] = (df['high'] - df['low']).rolling(14).mean() # Simple proxy if TA not avail
