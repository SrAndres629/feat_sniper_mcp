...
##### `remote new`
Creates a new remote session to delegate a task to Jules. Jules can automatically infer the repository from your current working directory, so you can often omit the `--repo` flag.

- `--repo <repo_name>`: Specifies the repository for the session.
...
Example:
```bash
# Start a new session to write unit tests
